...
impl<T: Trait> Module<T> {
	...
	fn fetch_price_and_send_signed() -> Result<(), &'static str> {
		...
		let price = Self::fetch_price().map_err(|err| { debug::error!("fetch price error: {:?}", err); "Failed to fetch price{}"})?;
	...
	fn fetch_price() -> Result<u32, rt_offchain::http::Error> {
		let deadline = sp_io::offchain::timestamp().add(rt_offchain::Duration::from_millis(10_000));
		...
		debug::info!("Got price of DOT/USDT: {} cents", price);

		Ok(price)
	}

	// fn parse_price2(resp_str: &str) -> Option<u32> {
	// 	let price: PricePayload =
	// 		serde_json::from_str(&resp_str).map_err(|_| "serde_json parse error")?;
	// 	Some(price.usd as u32)
	// }
	...
	fn parse_price(price_str: &str) -> Option<u32> {
		...
		Some((price.integer as u32 * 100 + (price.fraction / 10_u64.pow(exp)) as u32)/100 as u32)
		// Some(price.integer as u32)
	}

	/// Add new price to the list.
	fn add_price(who: T::AccountId, price: u32) {
		debug::info!("Adding to vector with new price: {}", price);
